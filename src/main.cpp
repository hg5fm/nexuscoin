...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uNXC
...
void ThreadScriptCheck() {
    RenameThread("nexuscoin-scriptch");
...
unsigned char pchMessageStart[4] = { 0xae, 0x5f, 0x2b, 0x85 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NexuscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NexuscoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("NexuscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("nexuscoin-miner");
    ...
    try { loop {
        ...
        printf("Running NexuscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NexuscoinMiner terminated\n");
        throw;
    }
}
