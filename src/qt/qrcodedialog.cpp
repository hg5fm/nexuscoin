...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("NXC")
    updateDisplayUnit();
}
...
{
    QString ret = QString("nexuscoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non NXC unit input in lnReqAmount, we generate the URI with NXC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::NXC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
